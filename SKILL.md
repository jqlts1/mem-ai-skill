...
保存到 Mem AI 的笔记时，默认不要使用三反引号代码块（如 ```bash、```text）。优先使用纯标题、普通列表、短段落和行内命令，避免内容在 Mem 或其它预览链路里被转义成 `&#96;&#96;&#96;` 这类符号，影响阅读和搜索。

## 标题命名约定（Mem 专用）

Mem 主要用于语义检索和向量召回，标题不必刻意短；相比“简短好看”，更优先“可检索、可回忆、可区分”。

默认规则：
- 标题可以适当长一点，优先写清 **主题 + 作用/用途 + 场景/任务词**
- 标题首先服务于后续搜索，不必追求营销感或过度精炼
- 如果是工具、方法、方案类内容，优先把“能用来做什么”写进标题
- 如果是带明显使用边界的内容，可补充 1 个关键限定词，帮助和相似笔记区分
- 标题仍保持朴素、直接、便于个人搜索
