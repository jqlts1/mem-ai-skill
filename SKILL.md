...
默认先用封装命令 `~/.openclaw/skills/mem-ai/scripts/memctl`，不要先手写 `curl`。
只有在以下场景才直接调用底层 API：
- 需要 `mem-it`
...
```bash
~/.openclaw/skills/mem-ai/scripts/memctl save "# 标题\n\n正文"
~/.openclaw/skills/mem-ai/scripts/memctl search "关键词" --limit 5
~/.openclaw/skills/mem-ai/scripts/memctl list --limit 5
~/.openclaw/skills/mem-ai/scripts/memctl read <note_id>
~/.openclaw/skills/mem-ai/scripts/memctl update <note_id> --file ./note.md --mode replace
~/.openclaw/skills/mem-ai/scripts/memctl delete <note_id>
```
