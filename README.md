...
- "搜索关于项目A的笔记" → `search`（语义检索 + 关键词）
- "看看我最近的笔记" → `list`（分页浏览）
- "我有哪些未完成的待办" → `search(contains_open_tasks=true)`

### 搜索策略（语义优先）

Mem 搜索支持向量语义检索，默认优先用自然语言表达完整意图，而不是机械堆关键词。

- 首选：一句自然语言描述
- 推荐：`自然语言一句话 + 2~4 个核心关键词`
- 多关键词场景：用空格分隔 3~8 个关键词（不要拼成一个长词）
- 不建议默认“先窄后宽”；通常先宽搜即可
- 仅在结果不理想时微调：
...
```bash
scripts/memctl search "我想找之前保存的 FastClaw 笔记，重点看用途、适用场景和风险"
scripts/memctl search "我想找之前保存的 FastClaw 笔记 用途 适用场景 风险 OpenClaw 托管"
```
...
以下相对路径默认都以当前 skill 根目录为基准。

```bash
# 保存笔记
scripts/memctl save "# 今日记录\n\n完成了 A/B 测试"

# 搜索笔记
scripts/memctl search "A/B 测试" --limit 5
...
scripts/memctl update <note_id> --file ./note.md
...
```bash
ln -sf "$(pwd)/scripts/memctl" ~/.local/bin/memctl
memctl list --limit 5
```
